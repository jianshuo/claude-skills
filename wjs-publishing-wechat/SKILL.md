...
帮助用户写微信公众号文章。**轻润色，不重写。** 自动生成题图和解释图，一行命令推草稿。
...
**不要做的事**：改用词偏好；加 AI 味儿连接词（"首先 / 其次 / 综上所述 / 值得注意的是"）；把口语改书面语；加 emoji；重组段落顺序；"提升"作者表达。

**改动尺度**：如果你改的字数超过原文 5%，你改太多了，退回去。

## 长度与例子（硬约束）

**默认 800–1000 字。** 第一稿就按预算写，不要写完再砍——砍出来的文章会残留拼接感。超 1200 回去再砍一轮。

写到例子段时过这把尺：**这个例子是真的具体（真事 / 真人 / 真数字），还是为了演示框架编出来的？** 后者直接删，让 `illustration.png` 自己承担"演示结构"的功能。

**优先保留**：开头钩子 + 核心框架 + 1 句点睛 + 软着陆结尾。
**优先砍掉**：演示性例子、重复阐释、"怎么用 / 入口在哪"这类 instructional 段落。

**默认不要写 `## 后注`**。除非真的有必要的致谢、来源标注，否则在正文最后落点收束即可。
...
`upload-draft.sh` 会把 `**...**` 渲染成**红色粗体**——这是作者刻意要的视觉重点。**每篇文章正文都必须有合理的加粗，一处都没有 = 没写完。**

规则：
- **2–4 处**，打在点睛句、关键结论、核心概念词上
- 优先加在：每节一句话结论、全文情绪落点、读者最该记住的那句
...
正文出现安装 / 运行命令时，**默认拉出来单独成段**，不要混在叙述句里写成 inline。两种写法：

- **首选**——淡底色代码块（raw HTML 块，整段一行，不能有内部空行）：
  ```
  ...
  ```
- 或 fenced ```` ```bash ```` 块，`upload-draft.sh` 转成独立的 `<p><code>…</code></p>`

句中只提一下命令名 → inline `` `code` `` 即可。

## 介绍 skill 的文章：末尾必须附安装方法

**触发条件**：这篇是在介绍 / 推荐某个具体的 Claude Code skill。
...
**末尾附下面这段**（把 `<SKILL_NAME>` 替换成实际 skill 名）：
...
3. Hermes 单独列**命令行**形式，因为它是 registry CLI 而非 chat agent
4. 最后那句「装完之后，对 agent 说一句『……』」根据当前 skill 的实际触发语写，**不要漏**
5. 通常放在文章最后；如果有 `## 后注` 放在后注之前

## Workflow

### Step 0: 接收输入

用户的输入形式：完整草稿 / 散乱思路 / 长段没分段 / 语音转写（可能有错字）。如果输入太散，**问一个问题**："这是想写一篇文章，还是几个独立想法？" —— 只问这一次。

### Step 1: 轻润色

- 修错字（"的得地"、同音字、"我我"重复字）
- 段落切分：每 1–3 句一段
- 拗口处做最小改动；改完语气变了，宁可不改
- 标点统一：中文全角，英文 / 数字之间空格
- 保留原本的开头和结尾

### Step 2: 标题候选

给用户 **3 个标题候选**：A) 直白型；B) 故事型；C) 用户原文里最有味道的一句。不做：标题党、夸张、"震惊"、"必看"。

### Step 3: 摘要 (50–80 字)

不是文章第一段的复制；一句话说清楚读者会获得什么；用作者的语气，不是营销腔。
...
**⚠️ 正文里除 `cover.png` / `illustration.png` 之外的图不会自动上 CDN。** 用户给的本地截图（如 `img-xxx.png`）需要每张先 `md2wechat upload_image img-xxx.png --json` 拿到 `data.wechat_url`，再替换 `article.md` 里的本地路径。验证：`grep -c mmbiz content.html` = 正文图片数，`grep -c 'img-' content.html` = 0。

默认插入位置：**正文最后落点之后**（有 `## 后注` 放后注前；有 `## 安装方法` 放安装方法前）。

**绝不要给解释图写引导语**——不写「整件事画起来是这样」「如图所示」之类。图自己说话。详见 [[no-illustration-caption]]。

> 安全网：如果 `illustration.png` 存在但 `article.md` 没引用，`upload-draft.sh` 会自动插入并改写 `article.md`，幂等。
...
3. 如果 `illustration.png` 存在但没引用，自动插入 `![](./illustration.png)` 到正文末尾（幂等安全网）；然后 upload 拿 CDN URL
4. 从 `article.md` 生成 `content.html` —— 见下面转换规则
5. 装 `draft.json`，调 `create_draft` 或（如 `publish.json` 有 `draft_media_id`）调 `draft/update` 原地更新
...
- `WECHAT_PUBLISH_NO_PUSH=1` —— 不自动 push 到 git
...
**常见 errcode**：
- `40164`：IP 不在白名单
- `45004`：`meta.json` 的 `summary` 为空 / 太短
- `40007`：老 `draft_media_id` 被后台删了 → 脚本自动 fallback 建新
...
- [ ] 包含 article.md、cover.png、illustration.png、meta.json、original.md
- [ ] meta.json 字段齐全
