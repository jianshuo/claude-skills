...
- **Downstream of `/wjs-video-segmentation`** — the segmentation skill
...
| Is | Is not |
|---|---|
| Everything that goes ON TOP of a video clip: cover, caption, chapter, illustration, CTA | Cutting / cropping a video (that's `/wjs-video-segmentation` + `/wjs-video-crop`) |
...
```
clip.mp4 + clip.zh-CN.burn.srt   (from /wjs-video-segmentation hand-off)
   ↓
...
**Generation:** use `/wjs-video-segmentation/scripts/make_cover.py`
...
Use this when you're coming directly from `/wjs-video-segmentation`
...
```bash
python3 ~/.claude/skills/wjs-video-overlay/scripts/scaffold.py spec.json
...
- **Burning libass subs on top of HTML/CSS captions.** Pick ONE caption
  ...
  captions, do NOT also burn subs in `/wjs-video-segmentation` —
...
- **`gpt-image-2-skill`** — the cover generator. `make_cover.py`
  ...
  is required.
- **`/wjs-video-upload`** — the next downstream after this skill
