...
Convert a video's orientation by **cropping** a narrow band from the source — not by physically rotating it. The crop window follows detected faces over time so the active speaker stays in frame. A `.rotate.json` sidecar records the crop plan; the original input is never modified.
...
- `<input>.rotate.json` — sidecar with the crop plan
- `<input>_rotated.mp4` — final cropped + scaled video

## Sidecar schema (`<input>.rotate.json`)

```json
{
  "_about": "wjs-video-rotate crop plan for cam_a.MOV. Source not modified.",
  "_help": {
