...
description: 吃一堑长一智 — 走完 5 步交互式反思（堑 → 自动输出 → 旧参数 → 新参数 → 替代动作），从「情绪复盘」推进到「行为训练」。Use when user reflects on a personal setback, mistake, or recurring pattern (反思, 复盘, 回顾, 总结教训, 吃一堑, 长一智, "这次又栽了", "怎么又这样", "为什么我总是…", "想开点都做不到", "知道道理但做不到"). For the user as a human, not for Claude's task post-mortems.
---
...
帮用户在每次"吃堑"之后，按 5 步走一遍：

**堑** → **自动输出** → **旧参数** → **新参数** → **替代动作**

不开练习菜单，不排打卡频次，不要每天冥想 10 分钟。最终只交付一条：**下次这件事再发生的那一秒，他具体做什么不一样**。
...
**The Iron Law**：知道道理 ≠ 临场用上 ≠ 默认反应。光把"道理"加到笔记里，是在重复作者吐槽过的状态——"两千条笔记，下次被怼还是先暴怒三秒"。
"道理都懂还是过不好这一生"
...
**为什么**：自动输出是 L3 权重的可观测投影。事后再聪明的总结，梯度都已经凉了，改不动 L3。

如果用户给的是事后视角（"我应该当时冷静一点"、"我那时候不该…"），**追问一次**："这是事后视角。回到那一秒——脑子里实际蹦出来的那句话是什么？"

---

### Step 3 — 这个输出背后的旧参数是什么？
...
这五行的内在关系一句话：**旧参数**要改的是这个 → **新参数**练的方向是这个 → **替代动作**就是那一秒练它的方式。前三行讲清楚"练什么"，后两行讲清楚"怎么练"。
...
**自动输出**：「他们不重视和我吃饭」
**旧参数**：别人状态不明时，我默认解释成"跟我有关"，很少先想"他们各自有自己的事"
...
- 为什么要训练这条新参数？因为旧参数是"信息缺失时默认跟我有关"
- 为什么这条旧参数被触发？因为自动输出冒出的是「他们不重视」
