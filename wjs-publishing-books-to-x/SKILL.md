...
- **我的书判定**：
  - `author == "王建硕"` → 是我的。
  - `author` 为空 → **不确定**（可能是我早期没署名的，也可能是别人没设名字）。首次遇到时把空署名书列出来问用户哪些是他的，确认结果记进 `state/mine.txt`（一行一个 slug），下次不再问。
  - 其他署名（蜜蜡 / Dennis / sissi / …）→ 别人的书，**不发**。
...
| 空署名不问就当成我的 | 可能是别人没设名字；确认一次记 mine.txt |
