...
- For the auto-edit / cut / PiP rendering step that comes AFTER sync, use **wjs-multicam-edit** (consumes these sidecars).
...
```json
{
  "_about": "Sync metadata for cam_b.MOV. Apply via ffmpeg -itsoffset. See wjs-multicam-sync SKILL.md for full schema.",
  "schema_version": 1,
  "source": "cam_b.MOV",
  "reference": "cam_a.MOV",
...
For `wjs-multicam-edit`, the EDL builder in `autoedit.py` ingests every `<input>.sync.json` automatically; you don't compose these flags by hand.
