...
- No SRT yet → run `/wjs-transcribe` then `/wjs-translate-srt` first.
- Source-language only TTS (rare; usually you translate first) → still use this skill, but pass the source SRT.
- Burn-in only, no audio change → skip to `/wjs-burn-subs`.
...
```bash
.venv/bin/python ~/.claude/skills/wjs-dub/scripts/dub.py [voice] [rate] [pitch]

# Mature Chinese contemplative female (Volcano):
.venv/bin/python ~/.claude/skills/wjs-dub/scripts/dub.py \
...
Output: `<source-stem>_<lang>_dub.mp4` (e.g., `entrevista_zh_dub.mp4`). This is the input for the next step — `/wjs-burn-subs/render.py` — which composites the final video.
...
.venv/bin/python ~/.claude/skills/wjs-dub/scripts/visual_diarize.py \
...
Save as `*.tagged.srt`. Keep the **clean** SRT (without tags) for downstream burn-in via `/wjs-burn-subs`.
...
- `<source-stem>_<lang>_dub.mp4` — video stream-copied from source, audio replaced with the time-aligned dub track. Drop-in input for `/wjs-burn-subs/render.py`.
- `dub_work/seg_NN.mp3` — per-cue TTS clips (kept for resume / per-cue regen).

## Downstream

- **`/wjs-burn-subs`** — to mix the original audio as a low-volume bed, burn the SRT, or both. The final encode happens there in one ffmpeg pass (no cascade). Pass `--video <source.mp4> --dub <source_lang_dub.mp4> [--srt <srt>]` to its `render.py`.
- The dub-only file (`*_<lang>_dub.mp4`) is technically a finished video and can ship as-is, but it sounds dubbed (because it is). Mixing the original underneath gives the "professional translation" feel — do that in `/wjs-burn-subs`.
