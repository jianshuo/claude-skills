...
description: Use when the user has a long-form video (interview / lecture / podcast / conversation) and a transcript SRT, and wants to extract 3–6 stand-alone topical short clips from it. This skill ONLY cuts and crops — it produces raw clips + per-clip SRTs as a hand-off package for downstream post-production (`/wjs-video-overlay`). Triggers — "切成几段", "分主题", "拆成短视频", "切片", "topic segments", "split into clips".
---
...
cropping** — it hands off the raw clips to `/wjs-video-overlay` for
...
| `segment.py` cuts; `/wjs-video-crop` reorients | An end-to-end "magic" pipeline |
...
| Hands off **raw cropped clips + per-clip SRTs** | Burned subtitles, covers, intros, CTAs (those live in `/wjs-video-overlay`) |
...
`slug` = kebab-case English (used in filenames). `title` uses `\n` for line break, 2 lines max, 8–12 Chinese chars per line. `cover_prompt` is consumed downstream by `/wjs-video-overlay`'s cover-generation step — keep it written here so the overlay skill can pick it up without re-asking.
...
`/wjs-video-overlay` captions expect (its compositions start the
...
`/wjs-video-overlay`'s HTML/CSS caption rendering.)

## Hand-off package — what to deliver to `/wjs-video-overlay`
...
Then invoke `/wjs-video-overlay` to add covers, captions, illustrations,
...
| Convert orientation (ask first) | invoke `/wjs-video-crop` per clip |
...
- **Burning subs / generating covers in THIS skill** — those moved to `/wjs-video-overlay`. This skill stops after Step 4.

## Integration with other skills

- **`/wjs-translate-video`** — produce the source SRT first if missing. The word-level Whisper output (or Volcano/豆包 ASR output) is preferred for accurate cue timing.
- **`/wjs-video-crop`** — call in Step 3 when source orientation doesn't match target platform. Face-tracked active-speaker following keeps the talker in frame.
- **`/wjs-multicam-edit`** — if the source is multi-cam, render the synced single MP4 first, then segment.
- **`/wjs-video-overlay`** — the **default downstream** for everything after Step 4. Covers, captions, illustrations, CTA, and final render all happen there. Don't add post-production in this skill.

## Files & references

- `scripts/segment.py` — accurate-seek + stream-copy cutting
- `scripts/burn_subs.py` — SRT slicer (`--no-burn` mode); legacy libass burn-in mode is deprecated in favor of `/wjs-video-overlay`
