# X Follower Growth — Scoreboard
_generated 2026-05-27_

## 📈 Where we stand

- **Latest day (2026-05-26)**: 180 visits, 73 new follows → ratio **0.406**
- **7-day median ratio**: **0.388**  (n=6 days)

## 🧪 Active experiments

| # | cat | experiment | metric | baseline | post | Δ | verdict | note |
|---|-----|-----------|--------|----------|------|---|---------|------|
| 1 | profile | Bio 重写：身份锚+硬证明+关注CTA | follows÷visits | 0.388 | — | — | ⏳ 8d left |  |

## ✅ To-do (backlog — proposed, not yet applied)

- [ ] **#2** (profile → follows÷visits): 头像换高清正脸  
      _hypothesis: 48px老缩略图换成清晰正脸照，提升第一眼信任→转化_
- [ ] **#3** (profile → follows÷visits): 显示名加身份锚（Jianshuo Wang 王建硕 · …）  
      _hypothesis: 名字带上身份/标签让陌生访客一眼知道你是谁，更愿意关注_
- [ ] **#4** (profile → follows÷visits): 置顶一条最强 thread  
      _hypothesis: 给没被预热的访客一个'代表作'落点，提升转化_

## 📚 Settled

_nothing settled yet._
